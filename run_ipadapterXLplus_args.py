...
def parse_args():
    parser = argparse.ArgumentParser(description="Run IPAdapterXLPlus with arguments")
    parser.add_argument('--base_model_path', type=str, default="RealVisXL_V1.0/", help='Path to base model')
    parser.add_argument('--image_encoder_path', type=str, default="IP-Adapter/models/image_encoder/", help='Path to image encoder')
    parser.add_argument('--ip_ckpt', type=str, default="IP-Adapter/sdxl_models/ip-adapter-plus_sdxl_vit-h.bin", help='Path to IP-Adapter checkpoint')
    parser.add_argument('--output_dir', type=str, default="output/vis_sdxl_plus/", help='Output directory')
    parser.add_argument('--scale', type=float, default=0.5, help='Scale parameter')
    ...
    parser.add_argument('--seed', type=int, default=28, help='Seed for generation')
...
tokenizer = CLIPTokenizer.from_pretrained(base_model_path, subfolder="tokenizer",use_safetensors=True,variant="fp16",)
tokenizer_2 = CLIPTokenizer.from_pretrained(base_model_path, subfolder="tokenizer_2",use_safetensors=True,variant="fp16",)
unet = Dift_UNet2DConditionModel.from_pretrained(
        base_model_path, subfolder="unet", torch_dtype=torch.float16, use_safetensors=True, variant="fp16",)
...
image = Image.open(reference_image_path).resize((resolution//2, resolution//2))
inversion_image = np.array(load_image(reference_image_path).resize((resolution, resolution)))

mask_image_path = args.mask_image_path
mask = Image.open(mask_image_path).convert("L")
mask = center_crop_resize(mask, crop_ratio=1.0, size=1024)
mask = np.array(mask)
mask = (mask > 127).astype(np.uint8)
mask = torch.from_numpy(mask).float()
mask = mask.unsqueeze(0)

cur_model = IPAdapterPlusXL
ip_model = cur_model(pipe, image_encoder_path, ip_ckpt=ip_ckpt, device=device, num_tokens=16)
...
ref_latents = inversion_pipeline_sdxl_ipadapter.ddim_inversion(pipe, inversion_image, ref_prompt, num_inference_step, 7.5, scale, prompt_embeds, negative_prompt_embeds, pooled_prompt_embeds, negative_pooled_prompt_embeds) #[0,1,2,...,50]
