...
class Demo:
    ...
    def __init__(self, imgs, ft, img_size):
        self.ft = ft.to(torch.float32) # N+1, C, H, W
        self.imgs = imgs
        ...
        def onclick(event):
            if event.inaxes == axes[0]:
                with torch.no_grad():
                    ...
                    gc.collect()
                    torch.cuda.empty_cache()  
                    norm = src_vec.norm().item()
                    display({
                        'norm': norm,
                        'max': src_vec.max().item(),
                        'min': src_vec.min().item(),
                    })
                    ...
                    raw_src = src_vec.detach().cpu() 
                    display({
                        'shape': raw_src.shape,
                        'mean': raw_src.mean().item(),
                        'std':  raw_src.std().item(),
                        'values': raw_src.numpy()         
                    })
                    axes[0].clear()
                    ...
                    for i in range(1, self.num_imgs):
                        ...
                        max_sim = heatmap[max_idx]
                        display(f'Target image {i}: Max cosine similarity = {max_sim:.4f} at {max_idx}')
                        display(np.max(heatmap) - np.min(heatmap))
