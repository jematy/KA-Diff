...
class OneStepSDPipeline(StableDiffusionXLPipeline):
    @torch.no_grad()
    def __call__(
        self,
        img_tensor,
        ...
        encoder_states = prompt_embeds if not isinstance(prompt_embeds, (list, tuple)) else prompt_embeds
...
class SDFeaturizer:
    def __init__(self, sd_id='stable-diffusion-xl-base-1.0/', null_prompt=''):
        # sd_id='stabilityai/stable-diffusion-2-1'
        sd_id = 'stable-diffusion-xl-base-1.0/'
        unet = MyUNet2DConditionModel.from_pretrained(sd_id, subfolder="unet", torch_dtype=torch.float16)
    ...
    def forward(self,
        ...
        prompt_embeds = prompt_embeds.repeat(ensemble_size, 1, 1).to(dtype=torch.float16, device='cuda')
...
class SDFeaturizer4Eval(SDFeaturizer):
    def __init__(self, sd_id='stable-diffusion-xl-base-1.0/', null_prompt='', cat_list=[]):
